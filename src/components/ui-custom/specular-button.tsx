...
export function SpecularButton({
  ...
  const content = (
    <>
      <span
        ref={fxRef}
        className={styles["specular-button__fx"]}
        aria-hidden="true"
      />
      <span className={styles["specular-button__label"]}>{children}</span>
    </>
  );

  const combinedClassName = `${styles["specular-button"]} ${
    styles[`specular-button--${size}`]
  }${className ? ` ${className}` : ""}`;
